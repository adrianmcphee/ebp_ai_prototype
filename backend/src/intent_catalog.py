...
BANKING_INTENTS = {
    ...
    "payments.transfer.internal": BankingIntent(
        ...
        example_utterances=[
            ...
            "Move $500 from checking to business account",
            "Transfer from savings to business",
            "Move money from business to checking",
        ...
        keywords=["transfer", "move money", "between accounts", "move to savings", "move to checking", "move to business", "move from checking", "move from savings", "move from business", "internal transfer", "transfers", "transfer hub", "money transfers", "transfers page"],
        patterns=[
            r"\btransfer .* (to|from|between) .* account\b",
            r"\bmove .* (to|from) (savings|checking|business)\b",
            r"\bmove .* from .* (to|into) .* (account|savings|checking|business)\b",
    ...
    "payments.transfer.external": BankingIntent(
        ...
        risk_level=RiskLevel.HIGH,
        auth_required=AuthLevel.FULL,
        required_entities=["amount", "recipient", "from_account"],
    ...
    "business.account.open": BankingIntent(
        ...
        example_utterances=[
            ...
            "Open company banking account",
            "New business checking account",
            "Apply for business account",
            "Set up business banking",
        ],
        keywords=["open business account", "business banking", "corporate account", "company banking", "new business account", "start business account"],
        patterns=[
            r"\b(open|start|create|new) .* business .* account\b",
            r"\bbusiness .* account .* (opening|application)\b",
            r"\b(open|start|create) .* corporate .* account\b",
            r"\bcompany .* banking .* (account|service)\b",
            r"\bnew .* business .* (checking|savings)\b",
        ],
