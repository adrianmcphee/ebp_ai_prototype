...
BANKING_INTENTS = {
    ...
    "navigation.accounts.details": BankingIntent(
        ...
        optional_entities=["account_type", "account_name"],
        example_utterances=[
            "Show my savings account",
            "Take me to my checking account", 
            "Show my primary checking",
            "Go to business checking account",
            "View my savings account details",
            "Go to account details",
            "View my account",
            "I want to see my savings account",
            "Open my checking account",
            "Show me my Business Checking account",
            "Display my account information",
            "Let me see my account balance and details",
        ],
        keywords=["take me to", "show my", "show me", "view my", "go to my", "navigate to", "open my", "display my", "want to see", "let me see", "my account", "account details", "account information", "account balance"],
        patterns=[
            r"\b(show|go to|view|take me to|navigate to|open|display)(?:\s+me)?\s+(?:my\s+)?(savings|checking|primary|business)(?:\s+checking|\s+savings)?\s+account\b",
            r"\b(show|view)(?:\s+me)?\s+my\s+\w+\s+(?:checking|savings|account)\b", 
            r"\b(take me to|go to|navigate to)\s+my\s+account\b",
            r"\b(want to see|let me see)\s+my\s+\w*\s*account\b",
            r"\baccount (details|information|balance)\b",
        ],
        preconditions=[],
        daily_limit=500,
        timeout_ms=1000,
    ...
    "international.wire.send": BankingIntent(
        ...
        description="Send international wire transfer",
        confidence_threshold=0.85,
        risk_level=RiskLevel.HIGH,
        auth_required=AuthLevel.CHALLENGE,
        required_entities=["amount", "recipient_country", "swift_code", "recipient_account"],
        optional_entities=["purpose", "correspondent_bank"],
        example_utterances=[
